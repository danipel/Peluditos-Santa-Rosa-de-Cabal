...
import { MapPin, Phone, MessageCircle, Plus, X, Home, PawPrint, AlertCircle, Search, ArrowLeft, Loader2 } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      await Promise.all([cargarReportes(), cargarAlbergue()]);
      setLoading(false);
    })();

    // Realtime: cualquier cambio en reportes se refleja para todos al instante
  ...
  async function guardarAlbergue(info) {
    ...
    if (error) setError("No se pudo guardar la info del albergue.");
    else {
      setAlbergue(info);
  ...
  function coincidencias(reporte) {
    ...
    const tipoBuscado = reporte.estado === "perdido" ? ["avistado", "en_albergue"] : ["perdido"];
    ...
    <div style={{ minHeight: "100vh", background: "#F6F5F2", fontFamily: "'Inter', system-ui, sans-serif", color: "#2A2A28" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: "1 1 160px" }}>
            ...
            <input
              ...
              style={{ width: "100%", padding: "8px 10px 8px 32px", borderRadius: 8, border: "1px solid #DAD6CC", fontSize: 13.5, boxSizing: "border-box" }}
            />
          </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
          {["todos", "perdido", "avistado", "en_albergue", "reunido"].map((t) => (
            <button
              key={t}
              ...
              style={{
                padding: "5px 12px",
                borderRadius: 999,
                border: "1px solid " + (filtroTipo === t ? "#1F3A34" : "#DAD6CC"),
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {filtrados.map((r) => (
              <Card key={r.id} reporte={r} coincidencias={coincidencias(r)} onCambiarEstado={cambiarEstado} />
            ))}
          </div>
        )}
      </div>
...
function Card({ reporte, coincidencias, onCambiarEstado }) {
    ...
    <div style={{ background: "#fff", border: "1px solid #DAD6CC", borderRadius: 12, overflow: "hidden" }}>
      <div style={{ display: "flex", gap: 12, padding: 12 }}>
        <div style={{ width: 72, height: 72, borderRadius: 10, background: "#EFEDE6", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
          {reporte.foto_url ? (
            <img src={reporte.foto_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          ) : (
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 6 }}>
            <div style={{ fontWeight: 700, fontSize: 14.5 }}>
              {reporte.nombre ? reporte.nombre : ESPECIES[reporte.especie]}{" "}
              <span style={{ fontWeight: 400, color: "#8A8A85", fontSize: 12.5 }}>{reporte.nombre ? `· ${ESPECIES[reporte.especie]}` : ""}</span>
            </div>
      ...
      {coincidencias.length > 0 && (
        <div style={{ background: "#FAF1D6", padding: "8px 12px", fontSize: 12.5, color: "#7A5F00", display: "flex", gap: 6 }}>
          ...
          <span>
            Posible coincidencia con {coincidencias.length} reporte{coincidencias.length > 1 ? "s" : ""} en el mismo sector.
          </span>
        </div>
      )}
      ...
      <div style={{ display: "flex", borderTop: "1px solid #EFEDE6" }}>
        ...
        <select
          ...
          style={{ flex: 1.2, border: "none", background: "transparent", fontSize: 12.5, fontWeight: 600, color: "#4A4A47", textAlign: "center", cursor: "pointer" }}
        >
    ...
    return (
      <Modal onClose={onClose} title="¡Reporte publicado!">
        <div style={{ fontSize: 13.5, lineHeight: 1.5 }}>
          <p>Tu reporte ya está visible para todos. Guarda este código por si necesitas identificarte luego para editarlo:</p>
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div>
          ...
          <select value={form.estado} onChange={(e) => set("estado", e.target.value)} style={inputStyle}>
            ...
            <option value="avistado">Avistado — lo vi pero no lo tengo</option>
